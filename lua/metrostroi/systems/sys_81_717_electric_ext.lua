...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717_Electric_EXT")
TRAIN_SYSTEM.MVM = 1
...
function TRAIN_SYSTEM:SolveAllInternalCircuits(Train, dT, firstIter)
    ...
    Train:WriteTrainWire(4, S["10AK"] * KV["10AK-4"] + KV["4-0"] * -10) --[[max(0,min(1,T[4])*KV["4-0"]*-10)]]
    ...
    elseif isLVZ then
        ...
        if isKSD then
            ...
            Train:WriteTrainWire(6, S["10AK"] * Train.K6.Value)
            Train:WriteTrainWire(8, --[[ math.max(0,--]] --[[ )--]]
                BO * (KV["10-8"] + ARS["8"] * RC1 + AVO["8"] * RC2) * Train.A41.Value) --10-KV-8 FIXME ARS
    ...
    if not isDot2 then Train.RPL:TriggerInput("Set", S["20A"] * (1 - Train.RPvozvrat.Value) * (Train.DR1.Value + Train.DR2.Value + (1 - Train.BV.State))) end
    ...
    S["10X"] = Train.LK4.Value + C(
        --[[ S["1N"]*P.PS+--]]
        RK == 1) * Train.LK2.Value
    ...
    if isMVM then Train.BV:TriggerInput("Disable", T[71] * Train.A66.Value) end
...
function TRAIN_SYSTEM:SolveInternalCircuits(Train, dT, firstIter)
    ...
    if firstIter then
        for i, v in ipairs(wires) do
            T[v] = min(Train:ReadTrainWire(v), 1)
        end
